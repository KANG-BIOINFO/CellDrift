...
def plot_pairwise_coefficients(adata, genes, cmap = cmap, ci_min = -30, ci_max = 30):
    ...
    ct.plot_coefficients(adata, ['ISG15', 'ISG20', 'ISG20L2'])
    '''
    df_mu = pd.DataFrame(data = adata.varm['multicomp_mu'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms']).copy()
    df_pval = pd.DataFrame(data = adata.varm['multicomp_pvals'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms']).copy()
    df_lci = pd.DataFrame(data = adata.varm['multicomp_lci'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms']).copy()
    df_uci = pd.DataFrame(data = adata.varm['multicomp_uci'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms']).copy()
...
def plot_top_perturbed_genes(adata, cell_type, n_genes = 10, p_threshold = 0.05):
    ...
    df_pvals = pd.DataFrame(data = adata.varm['multicomp_pvals'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms'])
    sign_genes = df_pvals.loc[df_pvals[coeff] < p_threshold, :].index.values

    df_mu = pd.DataFrame(data = adata.varm['multicomp_mu'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms']).loc[sign_genes,:]
    df_lci = pd.DataFrame(data = adata.varm['multicomp_lci'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms']).loc[sign_genes,:]
    df_uci = pd.DataFrame(data = adata.varm['multicomp_uci'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms']).loc[sign_genes,:]
...
def plot_coefficient_comparison(adata, coeffs, pairwise = True, errorbar = True, genes = 'All', genes_highlight = None, arrows = False, figsize = (14,6)):
    ...
    '''    
    if pairwise:
        df_mu = pd.DataFrame(data = adata.varm['multicomp_mu'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms'])
        df_pvals = pd.DataFrame(data = adata.varm['multicomp_pvals'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms'])
        df_lci = pd.DataFrame(data = adata.varm['multicomp_lci'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms'])
        df_uci = pd.DataFrame(data = adata.varm['multicomp_uci'], index = adata.var_names, columns = adata.uns['celldrift']['multicomp_terms'])
        df_interval = (df_uci - df_lci) / 2
